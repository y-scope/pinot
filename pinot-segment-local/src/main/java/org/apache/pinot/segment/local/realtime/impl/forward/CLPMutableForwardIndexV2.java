...
public class CLPMutableForwardIndexV2 implements MutableForwardIndex {
  ...
  public CLPMutableForwardIndexV2(String columnName, FieldSpec.DataType storedType,
  ...
  public void setString(int docId, String value) {
    ...
    try {
      ...
      encodedVars = _clpEncodedMessage.encodedVars;
    } catch (IOException e) {
      throw new IllegalArgumentException("Failed to encode message: " + value, e);
    }

    _totalNumberOfDictVars += dictVars.length;
    int encodedVarsLength = null != encodedVars ? encodedVars.length : 0;
    _totalNumberOfEncodedVars += encodedVarsLength;
    _maxNumberOfEncodedVars = Math.max(_maxNumberOfEncodedVars, encodedVarsLength);
    ...
    _encodedVarsFwdIndex.setLongMV(docId, null != encodedVars ? encodedVars : ArrayUtils.EMPTY_LONG_ARRAY);
  }
