...
public class CLPForwardIndexCreatorV3Test {
  private static final File TEMP_DIR =
      new File(FileUtils.getTempDirectory(), CLPForwardIndexCreatorV3Test.class.getSimpleName());
  ...
  public void testCLPWriter()
    ...
    CLPMutableForwardIndexV2 clpMutableForwardIndexV2 = new CLPMutableForwardIndexV2(column, _memoryManager);
    for (int i = 0; i < logLines.size(); i++) {
      clpMutableForwardIndexV2.setString(i, logLines.get(i));
    }

    // Create a immutable forward index from mutable forward index
    CLPForwardIndexCreatorV3 clpForwardIndexCreatorV3 =
        new CLPForwardIndexCreatorV3(TEMP_DIR, column, logLines.size(), clpMutableForwardIndexV2.getCLPStats());
    for (String logLine : logLines) {
      clpForwardIndexCreatorV3.putString(logLine);
    }
    clpForwardIndexCreatorV3.seal();
    clpForwardIndexCreatorV3.close();

    // Read from immutable forward index and validate the content
    File indexFile = new File(TEMP_DIR, column + V1Constants.Indexes.RAW_SV_FORWARD_INDEX_FILE_EXTENSION);
