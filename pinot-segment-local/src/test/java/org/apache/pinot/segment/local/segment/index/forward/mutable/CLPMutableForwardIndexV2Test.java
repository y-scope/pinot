...
public class CLPMutableForwardIndexV2Test {
  ...
  public void testString()
    ...
    try (CLPMutableForwardIndexV2 readerWriter = new CLPMutableForwardIndexV2("col1", FieldSpec.DataType.STRING,
        _memoryManager, initialCapacity)) {
      int rows = 3;
      List<String> logLines = new ArrayList<>();
      logLines.add(
          "INFO [PropertyCache] [HelixController-pipeline-default-pinot-(4a02a32c_DEFAULT)] "
              + "Event pinot::DEFAULT::4a02a32c_DEFAULT : Refreshed 35 property LiveInstance took 5 ms. Selective:"
              + " true");
      logLines.add(
          "INFO [PropertyCache] [HelixController-pipeline-default-pinot-(4a02a32d_DEFAULT)] "
              + "Event pinot::DEFAULT::4a02a32d_DEFAULT : Refreshed 81 property LiveInstance took 4 ms. Selective:"
              + " true");
      logLines.add(
          "INFO [ControllerResponseFilter] [grizzly-http-server-2] Handled request from 0.0"
              + ".0.0 GET https://0.0.0.0:8443/health?checkType=liveness, content-type null status code 200 OK");
      logLines.add(
          "INFO [ControllerResponseFilter] [grizzly-http-server-6] Handled request from 0.0"
